...
class NeuralNetworkTest < Test::Unit::TestCase
  def setup
    @neuron_a = Models::Neuron.new([-0.16595599, -0.70648822, -0.20646505])
    @neuron_b = Models::Neuron.new([0.44064899, -0.81532281, 0.07763347])
    @neuron_c = Models::Neuron.new([-0.99977125, -0.62747958, -0.16161097])
    @neuron_d = Models::Neuron.new([-0.39533485, -0.30887855, 0.370439])
    @neuron_e = Models::Neuron.new([-0.5910955, 0.75623487, -0.94522481, 0.34093502])

    @layer_a = Models::Layer.new([@neuron_a, @neuron_b, @neuron_c, @neuron_d])
    @layer_b = Models::Layer.new([@neuron_e])
